(function ($) {
    ...
    var imJs = {
        ...
        cdHeadlineInit: function () {
            // Initialize the cd-headline text rotation
            if ($('.cd-headline').length) {
                // Process each cd-headline element
                $('.cd-headline.clip').each(function() {
                    var $headline = $(this);
                    var $wrapper = $headline.find('.cd-words-wrapper');
                    
                    // Ensure the first element is visible
                    var $firstVisible = $wrapper.find('b').first();
                    $wrapper.find('b').removeClass('is-visible').addClass('is-hidden');
                    $firstVisible.removeClass('is-hidden').addClass('is-visible');
                    
                    // Start rotation with a unique interval for each wrapper
                    if (!$wrapper.data('rotating')) {
                        $wrapper.data('rotating', true);
                        setInterval(function() {
                            var $current = $wrapper.find('b.is-visible');
                            var $next = $current.next('b');
                            
                            if ($next.length === 0) {
                                $next = $wrapper.find('b').first();
                            }
